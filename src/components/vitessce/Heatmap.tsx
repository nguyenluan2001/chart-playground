...
} from "./gl/dist-tsc/index.js";
import { range, clamp, isEqual } from "lodash";
import {
	getLongestString,
	DEFAULT_GL_OPTIONS,
	...
	getDefaultColor,
	cleanFeatureId,
} from "./utils/other-utils/dist-tsc/index.js";
 ...
 */
const HeatmapVT = forwardRef((props, deckRef) => {
	const {
		uuid,
		theme,
	...
	useEffect(() => {
		...
		const newCellOrdering =
			!cellColors || cellColors.size === 0 ? obsIndex : Array.from(cellColors.keys());
	...
	const [longestCellLabel, longestGeneLabel] = useMemo(() => {
		...
		return [getLongestString(obsIndex), getLongestString([...featureIndex, ...cellColorLabels])];
	...
	useEffect(() => {
		...
		if (
			...
			const promises = range(yTiles).map((i) =>
				range(xTiles).map(async (j) =>
					workerPool.process({
						curr,
						tileI: i,
						tileJ: j,
	...
	const heatmapLayers = useMemo(() => {
		const usePaddedExpressions =
			uint8ObsFeatureMatrix && shouldUsePaddedImplementation(uint8ObsFeatureMatrix.length);
	...
	const axisTopLabelData = useMemo(
		() =>
			(!transpose && featureLabelsMap
				? axisTopLabels.map(
						(d) => featureLabelsMap.get(d) || featureLabelsMap.get(cleanFeatureId(d)) || d,
					)
				: axisTopLabels
			).map((d, i) => [i, axisTopDashes ? `- ${d}` : d]),
		[axisTopLabels, axisTopDashes, transpose, featureLabelsMap],
	);
	const axisLeftLabelData = useMemo(
		() =>
			(transpose && featureLabelsMap
				...
				: axisLeftLabels
			).map((d, i) => [i, axisLeftDashes ? `${d} -` : d]),
		[axisLeftLabels, axisLeftDashes, transpose, featureLabelsMap],
	);
	const cellColorLabelsData = useMemo(
		() => cellColorLabels.map((d, i) => [i, d && (transpose ? `${d} -` : `- ${d}`)]),
	...
	const cellColorsLayersList = useMemo(() => {
		...
		const result = cellColorsTilesList.map((cellColorsTiles, track) =>
			cellColorsTiles
				? cellColorsTiles.map(
						(tile, i) =>
							new PixelatedBitmapLayer({
	...
	function onHover(info, event) {
		...
		if (trackI === null || trackColI === null) {
			...
			const obsI = obsIndex.indexOf(
				transpose ? axisTopLabels[trackColI] : axisLeftLabels[trackColI],
			);
		...
		const obsI = obsIndex.indexOf(transpose ? axisTopLabels[colI] : axisLeftLabels[rowI]);
		const varI = featureIndex.indexOf(transpose ? axisLeftLabels[rowI] : axisTopLabels[colI]);
...
HeatmapVT.displayName = "Heatmap";

export default HeatmapVT;
