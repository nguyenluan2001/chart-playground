...
export default class BitmaskLayer extends XRLayer {
    ...
    */
    ...
    draw(opts) {
        const { uniforms } = opts;
        const { channelStrokeWidths, channelsFilled, channelOpacities, channelColors, channelsVisible, channelFeatureValueColormaps, channelFeatureValueColormapRanges, channelIsStaticColorMode, channelIsSetColorMode, hoveredCell, colorScaleLo, colorScaleHi, isExpressionMode, zoom, minZoom, maxZoom, zoomOffset, // TODO: figure out if this needs to be used or not
         } = this.props;
        ...
        if (textures && model) {
            ...
            model
                ...
                channelColormapFuncIndices: padWithDefault(channelFeatureValueColormaps.map(d => GLSL_COLORMAPS.indexOf(d)), 0, 
                // There are six texture entries on the shaders
