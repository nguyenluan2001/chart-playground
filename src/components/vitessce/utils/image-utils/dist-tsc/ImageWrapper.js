import { normalizeCoordinateTransformations, coordinateTransformationsToMatrix, coordinateTransformationsToMatrixForSpatialData, getNgffAxes, getNgffAxesForTiff, physicalSizeToMatrix, hexToRgb, getSourceFromLoader, canLoadResolution, getStatsForResolution, isInterleaved as isInterleavedUtil, } from '@vitessce/spatial-utils';
...
export default class ImageWrapper {
    ...
    _getModelMatrix() {
        ...
        if ('multiscales' in this.vivLoader.metadata) {
            ...
            if ('spatialdata_attrs' in this.vivLoader.metadata) {
                ...
                this.vivLoader.metadata.multiscales[0], coordinateSystem);
            }
            else {
    ...
    getMultiResolutionStats() {
        ...
        return Array.from({ length: loader.data.length })
            ...
            .map((_, resolution) => {
            const { height, width, depth, depthDownsampled, totalBytes, dims, } = getStatsForResolution(loader.data, resolution);
            return {
