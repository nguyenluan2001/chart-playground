...
import { group as d3_group, rollup as d3_rollup, index as d3_index, } from 'd3-array';
...
describe('root.ts', () => {
    describe('unnestMap works as expected', () => {
        ...
        it('Can flatten a nested Map with one level and an aggregation function', () => {
            expect(unnestMap(athletesBySport, ['sport', 'value'], (arr) => arr.length)).toEqual([
                {
                    sport: 'Boxing',
                    value: 1,
                },
        ...
        it('Works with d3.group to switch the order of the hierarchy levels', () => {
            const unnestResult = (unnestMap(athletesBySportAndNation, ['sport', 'nation', 'value'])
                .flatMap((v) => v.value));
            const nationThenSport = d3_group(unnestResult, (d) => d.nation, (d) => d.sport);
        ...
        it('Works with d3.index to go from Map -> array of objects -> Map', () => {
            ...
            expect(unnestMap(d3_index(unnestMap(m, ['sport', 'value']), (d) => d.sport), ['sport', 'value'], (d) => d.value)).toEqual([
                { sport: 'Boxing', value: 1 },
                { sport: 'Soccer', value: 2 },
                { sport: 'MMA', value: 3 },
                { sport: 'Basketball', value: 50 },
        ...
        it('Works with d3.rollup to go from Map -> array of objects -> Map', () => {
            ...
            expect(Array.from(d3_rollup(unnestMap(m, ['sport', 'value']), (D) => D[0].value, (d) => d.sport).entries())).toEqual(Array.from(m.entries()));
        });
    });
});
