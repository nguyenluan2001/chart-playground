...
export const getStatsForResolution = (loader, resolution) => {
    ...
    const dims = Object.fromEntries(labels.map((label, i) => ([label, shape[i]])));
...
export function coordinateTransformationsToMatrixForSpatialData(ngffMetadata, targetCoordinateSystem) {
    const { datasets, coordinateTransformations, axes, } = ngffMetadata; // In the image/labels case, this is multiscales[0].
