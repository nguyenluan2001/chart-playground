...
import { treeToSelectedSetMap, treeToObsIdsBySetNames, } from './cell-set-utils.js';
/**
 ...
 * @returns
 */
export function stratifyArrays(sampleEdges, sampleIdToObsIdsMap, sampleSets, sampleSetSelection, obsIndex, mergedCellSets, cellSetSelection, arraysToStratify, // Assumed to be sorted with respect to the obsIndex.
    ...
    cellSetKeys.forEach((cellSetKey) => {
        ...
        sampleSetKeys.forEach((sampleSetKey) => {
            ...
            if (obsIdsInSampleSet && obsIdsInCellSet) {
                subObsIndex = Array.from((new Set(obsIdsInSampleSet)).intersection(new Set(obsIdsInCellSet)));
            }
 ...
 * @returns
 */
export function stratifyExpressionData(sampleEdges, sampleSets, sampleSetSelection, expressionData, obsIndex, mergedCellSets, geneSelection, cellSetSelection, cellSetColor, // TODO: remove this parameter
 ...
 * @returns
 */
export function aggregateStratifiedExpressionData(stratifiedResult, geneSelection, featureAggregationStrategy) {
    const result = new InternMap([], JSON.stringify);
